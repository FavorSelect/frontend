...
const SearchResultsWrapper = () => {
  const { products, labels, isLoading, isError } = useSelector(
    (state: RootState) => state.imageSearch
  );
            ...
            {!isLoading && !isError && labels.length > 0 && (
              <div className="mb-5 flex flex-wrap gap-2 text-sm text-gray-700 items-center">
                ...
                {labels.map((label, idx) => (
                  <span
                    key={idx}
                    className="bg-[#fff1f1] text-scarlet-red px-3 py-1 rounded-full text-xm"
                  >
                    {label}
                  </span>
                ))}
